...
Autor: FinShark Project - TAG Simulation
Data: 2024-01-01
"""
...
# from tqdm import tqdm  # Não usado no modo simplificado
...
DATA_FIM = "2024-01-01 23:55:00"  # Um dia baseado nos dados disponíveis

# Diretórios dos dados (caminhos absolutos)
SWOT_DIR = "C:/Users/leand/Documents/ufu/Nasa/FinShark/FinStream/TAG/Data/swot_for_loop"
MODIS_DIR = (
...
# Arquivos removidos - não necessários para simulação única
# DADOS_AMBIENTAIS, OUTPUT_FILE e ANALISE_DIR removidos
...
def carregar_dados_ambientais_por_data(data):
    ...
    print(
        f"Encontrados {len(swot_files)} arquivos SWOT e {len(modis_files)} arquivos MODIS para {data}"
    )
    ...
    print(
        f"SSHA range: {df_intersecao['ssha'].min():.2f} a {df_intersecao['ssha'].max():.2f}"
    )
    print(
        f"Chlor_a range: {df_intersecao['chlor_a'].min():.4f} a {df_intersecao['chlor_a'].max():.4f}"
    )
...
def carregar_dados_ambientais():
    ...
    if len(df_ambiental) == 0:
        raise ValueError(
            f"Nenhum dado ambiental válido encontrado para {primeira_data}"
        )

    print(f"Dados carregados: {len(df_ambiental):,} pontos válidos")
    print(
        f"SSHA range: {df_ambiental['ssha'].min():.2f} a {df_ambiental['ssha'].max():.2f}"
    )
    print(
        f"Chlor_a range: {df_ambiental['chlor_a'].min():.4f} a {df_ambiental['chlor_a'].max():.4f}"
    )
...
def analisar_dados_ambientais_basico(df_ambiental):
    ...
    # Diretório de análise removido - não necessário
...
# Função salvar_dados_tubaroes_por_dia removida - não necessária para simulação única
...
def main():
    ...
    df_ambiental, kdtree, coords = carregar_dados_ambientais_por_data(data)
    if df_ambiental.empty:
    ...
    output_dir = (
        "C:/Users/leand/Documents/ufu/Nasa/FinShark/FinStream/" "TAG/Data/data_tag_fake"
    )
    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(output_dir, "tubarao_tag_simulado.csv")
    ...
    print(f"  CRC-16 válido: {len(df_final[df_final['crc16'] > 0]):,} " f"registros")
    ...
    print("  - Dados de sensores realistas (acelerômetro, giroscópio, " "temperatura)")
